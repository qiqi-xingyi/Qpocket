...
#SBATCH --job-name=qpr_ddp_eval
#SBATCH --account=PGS0423
#SBATCH --partition=debug
#SBATCH --cpus-per-task=1
#SBATCH --mem=8G
#SBATCH --time=00:10:00
#SBATCH --output=/fs/scratch/PGS0423/%u/diffdock/kras_7rpz_full/analysis_slurm_%j.out
#SBATCH --error=/fs/scratch/PGS0423/%u/diffdock/kras_7rpz_full/analysis_slurm_%j.err
...
"${DD_PYTHON}" "${RUN_ROOT}/input/analyze_diffdock_pocket_kras.py" \
  --native-protein "${RUN_ROOT}/input/7RPZ_chainA_protein.pdb" \
  --predicted-protein "${RESULT_DIR}/rank1_protein.pdb" \
  --native-ligand "${RUN_ROOT}/input/6IC.sdf" \
  --predicted-ligand "${RESULT_DIR}/rank1.sdf" \
